...
describe('Fuzzy', function() {
    describe("static method", function() {
        describe("findFuzzyTimeUnit", function() {
            it('should enumerate correctly...', function() {
                assert.equal(Fuzzy.Unit.YEAR, Fuzzy.findFuzzyTimeUnit("Y"));
                assert.equal(Fuzzy.Unit.MONTH, Fuzzy.findFuzzyTimeUnit("M"));
                assert.equal(Fuzzy.Unit.WEEK, Fuzzy.findFuzzyTimeUnit("W"));
                assert.equal(Fuzzy.Unit.DAY, Fuzzy.findFuzzyTimeUnit("D"));
                assert.equal(Fuzzy.Unit.HOUR, Fuzzy.findFuzzyTimeUnit("h"));
                assert.equal(Fuzzy.Unit.MINUTE, Fuzzy.findFuzzyTimeUnit("m"));
                assert.equal(Fuzzy.Unit.SECOND, Fuzzy.findFuzzyTimeUnit("s"));
            });
        });

        describe(".parseDuration", function() {
            it('should return matching array of maps', function() {
                var x = JSON.stringify([{time: 1, unit: Fuzzy.Unit.YEAR},
                    {time: 2, unit: Fuzzy.Unit.WEEK},
                    {time: 2, unit: Fuzzy.Unit.MINUTE},
                    {time: 3, unit: Fuzzy.Unit.SECOND}]);
                assert.equal(x, JSON.stringify(Fuzzy.parseDuration("P1Y2WT2M3S")));

                x = JSON.stringify([{time: 1, unit: Fuzzy.Unit.YEAR}]);
                assert.equal(x ,JSON.stringify(Fuzzy.parseDuration("P1Y")));

                x = JSON.stringify([{time: 2, unit: Fuzzy.Unit.MINUTE}]);
                assert.equal(x, JSON.stringify(Fuzzy.parseDuration("T2M")));

                x = JSON.stringify([{time: 2, unit: Fuzzy.Unit.MONTH}]);
                assert.equal(x, JSON.stringify(Fuzzy.parseDuration("2M")));

                x = JSON.stringify([{time: 2.5, unit: Fuzzy.Unit.MONTH}]);
                assert.equal(x, JSON.stringify(Fuzzy.parseDuration("P2.5M")));

                x = JSON.stringify([{time: 0.6, unit: Fuzzy.Unit.DAY}]);
                assert.equal(x, JSON.stringify(Fuzzy.parseDuration("0.6D")));
            });
        });
        ...
        describe("addTime", function() {
            it('should return "2013-01-02"', function() {
                ...
                assert.equal(Fuzzy.addTime(date, 1, Fuzzy.Unit.DAY).toUTCString(), target.toUTCString());
            });

            it('should return "2013-03-15 12:00"', function() {
                var date = new Date(2013, 00, 01);
                var target = new Date(2013, 02, 16, 12);
                assert.equal(Fuzzy.addTime(date, 2.5, Fuzzy.Unit.MONTH).toUTCString(), target.toUTCString());
            });

            it('should return "2013-01-01 12:45"', function() {
                var date = new Date(2013, 00, 01, 12, 00);
                var target = new Date(2013, 00, 01, 12, 45);
                assert.equal(Fuzzy.addTime(date, .75, Fuzzy.Unit.HOUR).toUTCString(), target.toUTCString());
            });
        });
    });
    ...
    describe('#at()', function() {
        ...
        it('should return string versino of "thirtyAgo"', function() {
            var fuzzy = new Fuzzy();
            fuzzy.at("45 seconds ago", [{time: 45, unit: Fuzzy.Unit.SECOND}]);
            var text = fuzzy.build(thirtyAgo, pretendNow);
        ...
        it('should return string version of "thirtyAgo"', function() {
            var fuzzy = new Fuzzy();
            fuzzy.at("10 seconds ago", [{time: 10, unit: Fuzzy.Unit.SECOND}]);
            var text = fuzzy.build(thirtyAgo, pretendNow);
            assert.equal(text, thirtyAgo.toLocaleString());
        });
        ...
        it('should return "last year"', function() {
           ...
           fuzzy.at("last year", [{time: 1, unit: Fuzzy.Unit.YEAR}]);
        ...
        it('should return "last month"', function() {
           ...
           fuzzy.at("last month", [{time: 1, unit: Fuzzy.Unit.MONTH}]);
        ...
        it('should return "month and a day ago"', function() {
           ...
           fuzzy.at("month and a day ago", [{time: 1, unit: Fuzzy.Unit.MONTH}, {time: 1, unit: Fuzzy.Unit.DAY}]);
        ...
        it('should return "a year ago" and then "two years ago"', function() {
           ...
           fuzzy.at("a year ago", [{time: 12, unit: Fuzzy.Unit.MONTH}]);
           fuzzy.at("a year to go", [{time: -12, unit: Fuzzy.Unit.MONTH}]);
           fuzzy.at("two years ago", [{time: 24, unit: Fuzzy.Unit.MONTH}]);
           fuzzy.at("eight years ago", [{time: 96, unit: Fuzzy.Unit.MONTH}]);
           fuzzy.at("eight and a half years ago", [{time: 8, unit: Fuzzy.Unit.YEAR}, {time: .5, unit: Fuzzy.Unit.YEAR}]);
    ...
    /*describe("#parseTimeString", function() {
        it('should return [[30, "seconds"]]', function() {
            var result = Fuzzy.parseTimeString("30 seconds");
            assert.equal(result[0].value, 30);
            assert.equal(result[0].unit, Fuzzy.Unit.SECOND);
        });

        it('should return [[30, "s"], [45, "mins"]]', function() {
            var result = Fuzzy.parseTimeString("30 s 45 mins");
            assert.equal(result[0].value, 30);
            assert.equal(result[0].unit, Fuzzy.Unit.SECOND);
            assert.equal(result[1].value, 45);
            assert.equal(result[1].unit, Fuzzy.Unit.MINUTE);
        });
    });*/
